...
for ID in range(0, 6):
    ...
    position_dict["filename"] = "platooning_measurements/5_cars_vel_{}".format(str(velocity_dict["max_vel_first"]))

    # update the velocity of first (overtaking)
    velocity_dict["max_vel_first"] = 0.1 + 0.05 * ID
    velocity_dict["max_vel_second"] = (0.1 + 0.05 * ID)*1.1
    velocity_dict["max_vel_third"] = (0.1 + 0.05 * ID)*1.1
    velocity_dict["max_vel_fourth"] = (0.1 + 0.05 * ID)*1.1
    velocity_dict["max_vel_fifth"] = (0.1 + 0.05 * ID)*1.1
    ...
    platoons = []
    for namespace in namespaces:
        cmd_platooning = "exec rosrun platooning platooning_with_kick.py"
        try:
